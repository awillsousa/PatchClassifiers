...
def main():
    ...
    if os.path.isfile(arquivo_pesos):    
        model = util.model_fase01(INPUT_SHAPE, NUM_CLASSES+1, arquivo_pesos)
    else:    
    ...
    for id_img,arq_img in enumerate(arqs_imgs):
        ...
        prob_pred = np.max(pred_proba[np.where(pred_classes != NUM_CLASSES)])
        ...
        # Gerar imagem com as marcacoes dos patches votantes e não-votantes
        l_vota = patches[pred_classes[np.where(pred_classes != NUM_CLASSES)]]
        ##l_naovota = patches[pred_classes[np.where(pred_classes == NUM_CLASSES)]]
        util.marca_patches(arq_img, l_vota, legenda="V", tam_patch=64, rgb=True, saida="plots/ptxs-votantes.png")
