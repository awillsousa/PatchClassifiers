...
def load_breakhis(data_path='/home/willian/bases/cancer/', fold='fold1', mag='400X'):
    # Dados de treino
    arqs_tr = util.lista_imgs(dir_imagens=data_path+fold+'/train/'+mag)
    x_treino, y_treino, imagens_tr = util.prep_bases2(arqs_tr, 64, 64, gera_dic=True)
    x_treino = np.asarray(x_treino)
    y_treino = np.asarray(y_treino)
    print ("Amostras de treino: %d"%x_treino.shape[0])

    # Dados de teste
    arqs_ts = util.lista_imgs(dir_imagens=data_path+fold+'/test/'+mag)
    x_teste, y_teste, imagens_ts = util.prep_bases2(arqs_ts, 64, 64, gera_dic=False)
    x_teste = np.asarray(x_teste)
    y_teste = np.asarray(y_teste)
    print ("Amostras de teste: %d"%x_teste.shape[0])

    x = np.concatenate((x_treino, x_teste)).astype(float)
    ...
    np.save(data_dir + '/breakhis_features.npy', features)
    print('BreakHis - atributos salvos em ' + data_dir + '/breakhis_features.npy')
...
def load_data(dataset_name, dataset_path=None):
    if dataset_name == 'mnist':
        ...
        return load_breakhis(dataset_path)
    else:
