...
def filtra_base(base_tr, rotulos_tr, atrs_glcm, filtro):    
    ...
    if base_tr.shape[0] == 0 or rotulos_tr.shape[0] == 0:
        loga_sai("A base a ser filtrada não pfor part in pop:ode ser vazia")
    ...
    for l in range(linhas):         
        condicao = atrs_glcm[l][IDX_FILTRO[0]] < filtro[0]
...
def processa_resultados(resultados):    
    ...
    for r in resultados:
        if r == None:
            ...
            loga_sai("Encerrando programa prematuramente")
        else:
...
if __name__ == "__main__":
    ...
    #BASE_TR, ROTULOS_TR, QID = BASE_TR, ROTULOS_TR, QID
    ...
    #print("Base GLCM filtrada - tamanho: {0}".format(str(BASE_GLCM.shape)))
    ...
    for g in range(GEN):        
        ...
        p = mp.pool.Pool(20)
        vals_fits = p.map(toolbox.evaluate, pop)
        p.close()
        p.join()
        
        # coleta os resultados    
        for i, part in enumerate(pop):
            part.fitness.values = vals_fits[i]             
    ...
    print("Melhor base obtida: {0}".format(str(global_best.idxbase)))
    print("Tamanho da melhor base obtida: {0}".format(len(global_best.idxbase)))
    ...
    r = classifica.classificacao_base(BEST_BASE, BEST_ROTS, arq_base_ts, arq_base_ts.replace(".svm",".ppi"), "svm")
    ...
    # Plota curva ROC
    id_arq_roc = arq_log.replace(".log", "")
    helper.plot_roc(r['fpr'],r['tpr'],r['auc'], id_arq_roc, id_arq_roc)
