...
def classifica_img_proba(imagem, clf, atrib_ts):
    ...
    #logging.info("Formato atributos: {0}".format(atrib_ts.shape))
...
def classifica_img_proba_voto(imagem, clf, atrib_ts):
    logging.info("Classificacao imagem " + imagem)
...
'''
def classificacao_probas(atrib_tr, rotulos_tr, base_ts, arq_ppi, id_clf, idx_filtro, val_filtro):
    inicio = time()    
    imagens = {}
    ...
    try: 
        ...
        # Carrega a base de atributos GLCM
        arq_base_glcm = arq_base_tr.replace(".svm",".glcm")                               
        ATRIB_GLCM_TS, _,_ = carrega_base(arq_base_glcm,n_features=14)   
                        
        # Normaliza a base GLCM
        ATRIB_GLCM_TS = ATRIB_GLCM_TS.toarray()
        ATRIB_GLCM_TS = MinMaxScaler().fit_transform(ATRIB_GLCM_TS)
        ...
        for imagem in imagens:
            ...
            if idx2 > 0:
                ...
                logging.info("Total de atributos da imagem: {0}".format(atribs_img.shape[0]))
                logging.info("Tamanho atributos GLCM: {0}".format(ATRIB_GLCM_TS.shape))
                logging.info("Indices: {0}".format((idx1,idx2)))
                atrs_glcm = ATRIB_GLCM_TS[idx1:idx2]
                
                # Filtra a base passada            
                idxs = []
                
                for l in range(idx1,idx2): 
                    condicao = ATRIB_GLCM_TS[l][idx_filtro] < val_filtro
                    idxs.append(condicao)        
                            
                idxs = np.where(idxs)[0]
                base_filt = atribs_img[idxs, :]
                logging.info("Total de atributos FILTRADOS: {0}".format(base_filt.shape[0]))
                tst, pred, erro, prob = classifica_img_proba(imagem['arquivo'], clf, base_filt)
                r_tst.append(tst)
                r_pred.append(pred)
                total_erro += erro
                probs_imgs.append(prob)
                total_desc += (atribs_img.shape[0]-base_filt.shape[0])
                idx1 = idx2
...
def filtra_clasf(BASE_TR, ROTULOS_TR, BASE_GLCM, idx_glcm, val_filtro):
    ...
    if tam_base > 0:
        r = classificacao_probas(base_filt, rots_filt, base_teste, base_teste.replace(".svm",".ppi"), "rf", idx_glcm, val_filtro)    
...
if __name__ == "__main__":
    ...
    for mag, folds in bases.items():        
        for fold, bases in folds.items():
            ...
            helper.plota_grafico(vals_glcm, tams_glcm, arquivo="{0}-{1}-desc-tam".format(mag,fold), titulo="{0} - {1} Qtd. Patches".format(mag,fold), tituloX="Atrib. GLCM", tituloY="Tam. da Base (x1000)", legendas=leg)
            helper.plota_grafico(vals_glcm, taxas_fold, arquivo="{0}-{1}-desc-clf".format(mag,fold), titulo="{0} - {1} Classificação".format(mag,fold), tituloX="Atrib. GLCM", tituloY="Tx. Classificação", legendas=leg, plt0_1=True)
            helper.plota_grafico(vals_glcm, aucs_fold, arquivo="{0}-{1}-desc-auc".format(mag,fold), titulo="{0} - {1} AUC".format(mag,fold), tituloX="Atrib. GLCM", tituloY="AUC", legendas=leg, plt0_1=True)
