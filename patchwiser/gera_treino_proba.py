...
CLFS = {'knn':('KNN', KNeighborsClassifier(3)), 
        ...
        'rf':('Random Forest', RandomForestClassifier(n_estimators=50, max_depth=10, n_jobs=3)),
...
A probabilidade final da imagem é dada pelo produto da predição do primeiro mais (1 - predicao do segundo)

'''
def classifica_img_proba(imagem, clf, clf_rej, atrib_ts):
    ...
    preds_rej = clf_rej.predict_proba(atrib_ts)    
    pesos = preds_prob + (1-preds_rej)    
    probs_img = np.multiply(pesos[:,0], pesos[:,1])
    #print("Probabilidade imagem: {0}".format(str(probs_img)))
...
'''
def classificacao_probas(atrib_tr, rotulos_tr, atrib_rej, rotulos_rej, base_ts, arq_ppi, id_clf):
    inicio = time()    
    imagens = {}
    clf = SVC(gamma=0.5, C=32, cache_size=500, probability=True)   
    clf_rej = SVC(gamma=0.5, C=32, cache_size=500, probability=True)
    ...
    try: 
        ...
        logging.info("Treinando classificador de patches rejeitados...")
        clf_rej.fit(atrib_rej, rotulos_rej)                    
        ...
        for imagem in imagens:
            ...
            if idx2 > 0:
                ...
                tst, pred, erro, prob = classifica_img_proba(imagem['arquivo'], clf, clf_rej, atribs_img)
                r_tst.append(tst)
                r_pred.append(pred)
                total_erro += erro
...
def seleciona_patches(atrib_tr, rotulos_tr, clf, limiar):
    ...
    for tr_idx, ts_idx in skf.split(atrib_tr, rotulos_tr):
        ...
        # Seleciona os patches que foram rejeitados baseado no vetor de indices negado
        not_idxs = np.logical_not(idxs)
        rejeitados.append(X_ts[not_idxs,:])        
        rots_rejeitados.append(y_ts[not_idxs])
        
    return (melhores, rots_melhor, rejeitados, rots_rejeitados)
...
def executa_classificacao(base_tr, base_ts, opt_clf, arq_log, limiar=0.9): 
    ...
    rejeitados = []     # lista dos patches rejeitados (menos discriminativos)
    rots_rejeitados = []    # lista dos rotulos dos patches rejeitados
    ...
    clf = get_clf(opt_clf)
    melhores, rots_melhor, rejeitados, rots_rejeitados = seleciona_patches(atrib_tr, rotulos_tr, clf, limiar)    
    atrib_rej, rotulos_rej = agrega_base(rejeitados, rots_rejeitados)
    ...
    r = classificacao_probas(csr_matrix(atrib_red), rotulos_red, csr_matrix(atrib_rej), rotulos_rej, base_ts, arq_ppi_ts, opt_clf)
    r['descartados'] = reducao
    # Gera visualizacao das bases de patches reduzida e de patches rejeitados
    base_rdz = {"data":atrib_red, "labels": rotulos_red}
    base_rjt = {"data":atrib_rej, "labels": rotulos_rej}        
    helper.visualiza_bhtsne(base_rdz, id_visualiz+"-rdz")
    helper.visualiza_bhtsne(base_rjt, id_visualiz+"-rjt")
...
def main():     
    ...
    processa_resultados(resultados, arq_clf, options.opt_clf, idarq)
